...
export default function CSVImport() {
  ...
  async function handleImport() {
    setLoading(true);
    const { data } = await api.post('/csv/import', { rows: preview.allRows, mapping, filename: preview.filename });
    setResult(data);
    setLoading(false);
  }
      ...
      {preview && !result && (
        <>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="secondary" onClick={() => { setPreview(null); setResult(null); }}>Cancel</Button>
          </div>
        </>
      )}
