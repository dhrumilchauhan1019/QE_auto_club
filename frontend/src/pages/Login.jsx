...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setModalMessage(`Welcome back, ${data.user.name}!`);
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-3 gap-2 mb-5">
            {ROLES.map(r => (
              <button
                ...
                key={r.key}
                onClick={() => pickRole(r)}
                className={`flex flex-col items-center gap-1 py-3 rounded-lg border text-xs transition-colors ${role === r.key ? 'border-copper bg-copper/10 text-copper' : 'border-border text-slate hover:text-mist hover:border-steelLight'
                  }`}
              >
      ...
      <StatusModal
        ...
        buttonText={modalType === "success" ? "Continue" : "Try Again"}
